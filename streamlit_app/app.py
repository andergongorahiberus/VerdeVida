"""Verde Vida Nursery Agent — interfaz Streamlit con ciclo LLMOps integrado."""
...
def _render_scores_html(scores: dict[str, float]) -> str:
    ...
    for name, value in scores.items():
        ...
        css_class = (
            "score-good" if value >= 0.8 else ("score-warn" if value >= 0.5 else "score-bad")
        )
...
def _build_base_agent() -> object:
    from verde_vida_agent import create_agent
...
def _build_instrumented_agent() -> object:
    from verde_vida_agent.solution.observability import create_observed_agent
...
def _call_agent(user_input: str) -> tuple[str, float, dict[str, float]]:
    ...
    if mode == "instrumented":
        try:
            ...
            from verde_vida_agent.solution.prompt_provider import (
            ...
            from verde_vida_agent.solution.observability import process_query
...
def _show_prompt_dialog(label: str) -> None:
    try:
        from verde_vida_agent.solution.prompt_provider import get_prompt_client
        ...
        from verde_vida_agent.solution.prompt_provider import FALLBACK_PROMPT
...
def _tab_evaluation() -> None:
    ...
    if st.button(
        ...
        with st.status(f"Evaluando label `{eval_label}`…", expanded=True) as status:
            try:
                ...
                from verde_vida_agent.evaluation import run_evaluation
    ...
    if not st.session_state.langfuse_enabled:
        st.warning(
            "Langfuse no configurado. Define LANGFUSE_PUBLIC_KEY y LANGFUSE_SECRET_KEY en .env.",
            icon="⚠️",
        )
...
def _tab_cicd() -> None:
    ...
    with cicd_push:
        ...
        st.caption(
            ...
            "Equivale a ejecutar `python -m verde_vida_agent.cicd.push_prompt`."
        )
        ...
        if st.button(
            ...
            try:
                from verde_vida_agent.cicd.push_prompt import push_prompt
    ...
    with cicd_promote:
        ...
        st.caption(
            ...
            "mover la label en la UI de Langfuse o ejecutar `python -m verde_vida_agent.cicd.promote_prompt`."
        )
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary",
            disabled=not (st.session_state.langfuse_enabled and from_label != to_label),
        ):
            try:
                from verde_vida_agent.cicd.promote_prompt import promote_prompt
