...
-----------------------------------------
``verde-vida-system-prompt``  — the main system prompt for the agent.
...
# ---------------------------------------------------------------------------
FALLBACK_PROMPT = """\
Eres Vera, una asesora de atención al cliente del vivero Verde Vida.

## Ámbito
SOLO puedes ayudar con:
- Productos del catálogo de Verde Vida (precios, disponibilidad, tipos de plantas y árboles)
- Políticas del vivero (envíos, devoluciones, garantía, pagos, horarios, cuidados)
...
4. Si la consulta no es sobre productos o políticas de Verde Vida, responde:
   "Lo siento, solo puedo ayudarte con consultas sobre Verde Vida."
...
def _score_trace_online(
    ...
    """
    from verde_vida_agent.evaluation.evaluators import (
...
@observe(name="verde_vida_query_with_prompt")
def process_query_with_prompt(
    ...
    *,
    ...
    the observability patterns from ``verde_vida_agent.solution.observability``,
    this function:
    ...
    See Also:
        - ``verde_vida_agent.solution.observability.process_query`` — simpler
