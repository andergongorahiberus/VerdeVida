"""Fully-observed Verde Vida nursery agent — reference solution for LLMOps Notebook 2.
...
from verde_vida_agent.config import SYSTEM_PROMPT
from verde_vida_agent.tools import (
    _check_stock_impl,
...
def create_observed_agent(system_prompt: str | None = None) -> Agent:
    """Create a Verde Vida nursery agent that uses the observed tool wrappers.
    ...
    *,
    ...
    Args:
        ...
        system_prompt: Optional system prompt override.  Typically obtained
            from ``verde_vida_agent.solution.prompt_provider.get_system_prompt()``.
