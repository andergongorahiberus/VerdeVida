"""Verde Vida nursery agent tools — product search and FAQ lookup."""
...
from verde_vida_agent.config import load_catalog, load_faqs
...
def _search_catalog_impl(query: str) -> str:
    ...
    for product in catalog:
        # Include tipo (trees) and subtipo (plants) in the searchable text
        tipo = product.get("tipo", product.get("subtipo", ""))
        searchable = _normalize(f"{product['nombre']} {product['categoria']} {tipo}")
...
def _get_product_recommendations_impl(category: str, max_price: float) -> str:
    ...
    matches = [
        ...
        for p in catalog
        if (
            _normalize(p["categoria"]) == cat_norm
            or _normalize(p.get("tipo", "")) == cat_norm
            or _normalize(p.get("subtipo", "")) == cat_norm
        )
        and p["precio"] <= max_price
    ]
...
def get_faq_answer(topic: str) -> str:
    """Consulta las preguntas frecuentes del vivero Verde Vida.

    Args:
        topic: Tema a consultar (ej: devoluciones, envíos, garantía, pagos, cuidados).
...
def get_product_recommendations(category: str, max_price: float) -> str:
    ...
    Args:
        category: Categoría de producto (ej: arboles, plantas, frutal, interior, huerta, aromatica).
