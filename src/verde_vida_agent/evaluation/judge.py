...
def _build_ground_truth_summaries() -> tuple[str, str]:
    ...
    catalog_lines = [f"- {p['nombre']}: {p['precio']}€, category={p['categoria']}" for p in catalog]
    catalog_summary = "\n".join(catalog_lines)

    faqs = load_faqs()
    faq_lines = [f"- Q: {f['pregunta']} → A: {f['respuesta']}" for f in faqs]
    faq_summary = "\n".join(faq_lines)
...
def _get_ground_truth() -> tuple[str, str]:
    global _catalog_summary, _faq_summary
